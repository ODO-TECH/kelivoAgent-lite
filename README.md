...
## KelivoAgent-lite additions

This repository is a community fork based on [Kelivo](https://github.com/Chevey339/kelivo), maintained for Windows PC agent workflows. It keeps the upstream chat client and adds the following features:

- **Pi Agent integration** with an optional, user-controlled Pi Agent kernel update.
- **Workspace-first agent workflows**: create or link a workspace, choose a working directory, browse files, and run approved tools in the workspace.
- **Selectable desktop shells**: PowerShell, Command Prompt, Git Bash, or a custom executable path.
- **Codex-style composer syntax**: `/command` and `$skill-name` references are recognized and highlighted while preserving the original text sent to the model.
- **Built-in skills** including Skill Creator and PaddleOCR, with skill discovery and workspace-aware skill prompts.
- **Prompt optimization** through a configurable optimization model.
- **Additional providers and services** including Aliyun voice models and SiliconFlow model support.

The desktop pet experiment has intentionally been removed from this fork. The Windows build does not ship or launch a separate pet process.

The desktop build is the primary target of this branch. Mobile and other platform-specific code remains in the upstream-compatible source tree but is not the focus of this distribution.
