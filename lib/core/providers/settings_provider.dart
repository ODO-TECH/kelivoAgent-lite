...
class SettingsProvider extends ChangeNotifier {
  ...
  static const String _promptOptimizeModelKey = 'prompt_optimize_model_v1';
  static const String _promptOptimizePromptKey = 'prompt_optimize_prompt_v1';
  ...
  static const String _paddleOcrEnabledKey = 'paddleocr_enabled_v1';
  static const String _paddleOcrApiKey = 'paddleocr_api_key_v1';
  static const String _piAgentAutoUpdateKey = 'pi_agent_auto_update_v1';
  ...
  Future<void> _load() async {
    ...
    // load prompt optimizer model and prompt
    final optimizeSel = prefs.getString(_promptOptimizeModelKey);
    if (optimizeSel != null && optimizeSel.contains('::')) {
      final parts = optimizeSel.split('::');
      if (parts.length >= 2) {
        _promptOptimizeModelProvider = parts[0];
        _promptOptimizeModelId = parts.sublist(1).join('::');
      }
    }
    final optimizePrompt = prefs.getString(_promptOptimizePromptKey);
    _promptOptimizePrompt =
        (optimizePrompt == null || optimizePrompt.trim().isEmpty)
        ? defaultPromptOptimizePrompt
        : optimizePrompt;
    ...
    _paddleOcrEnabled = prefs.getBool(_paddleOcrEnabledKey) ?? true;
    _paddleOcrApiKeyValue = prefs.getString(_paddleOcrApiKey) ?? '';
    _piAgentAutoUpdate = prefs.getBool(_piAgentAutoUpdateKey) ?? false;
    // load summary model
  ...
  // Prompt optimizer model and instruction. This is intentionally separate
  // from the chat model so users can choose a fast, inexpensive editor.
  String? _promptOptimizeModelProvider;
  String? _promptOptimizeModelId;
  String? get promptOptimizeModelProvider => _promptOptimizeModelProvider;
  String? get promptOptimizeModelId => _promptOptimizeModelId;
  String? get promptOptimizeModelKey =>
      (_promptOptimizeModelProvider != null && _promptOptimizeModelId != null)
      ? '${_promptOptimizeModelProvider!}::${_promptOptimizeModelId!}'
      : null;

  static const String defaultPromptOptimizePrompt =
      '''You are a prompt editor. Rewrite the user's draft so another AI can execute it accurately.
Preserve the user's intent, facts, placeholders, code, and requested language. Clarify ambiguity only when the surrounding text supports it. Keep simple requests concise. Return only the rewritten prompt, without explanations or quotation marks.

<draft>
{content}
</draft>''';

  String _promptOptimizePrompt = defaultPromptOptimizePrompt;
  String get promptOptimizePrompt => _promptOptimizePrompt;

  Future<void> setPromptOptimizeModel(
    String providerKey,
    String modelId,
  ) async {
    _promptOptimizeModelProvider = providerKey;
    _promptOptimizeModelId = modelId;
    notifyListeners();
    await _preferences.setString(
      _promptOptimizeModelKey,
      '$providerKey::$modelId',
    );
  }

  Future<void> resetPromptOptimizeModel() async {
    _promptOptimizeModelProvider = null;
    _promptOptimizeModelId = null;
    notifyListeners();
    await _preferences.remove(_promptOptimizeModelKey);
  }

  Future<void> setPromptOptimizePrompt(String prompt) async {
    _promptOptimizePrompt = prompt.trim().isEmpty
        ? defaultPromptOptimizePrompt
        : prompt;
    notifyListeners();
    await _preferences.setString(
      _promptOptimizePromptKey,
      _promptOptimizePrompt,
    );
  }

  Future<void> resetPromptOptimizePrompt() async =>
      setPromptOptimizePrompt(defaultPromptOptimizePrompt);
...
Do not interpret or translate—only transcribe and describe what is visually present.''';
  ...
  bool _paddleOcrEnabled = true;
  String _paddleOcrApiKeyValue = '';
  bool get paddleOcrEnabled => _paddleOcrEnabled;
  String get paddleOcrApiKey => _paddleOcrApiKeyValue;
  bool _piAgentAutoUpdate = false;
  bool get piAgentAutoUpdate => _piAgentAutoUpdate;

  Future<void> setPiAgentAutoUpdate(bool value) async {
    if (_piAgentAutoUpdate == value) return;
    _piAgentAutoUpdate = value;
    notifyListeners();
    await _preferences.setBool(_piAgentAutoUpdateKey, value);
  }

  Future<void> setPaddleOcrEnabled(bool value) async {
    if (_paddleOcrEnabled == value) return;
    _paddleOcrEnabled = value;
    notifyListeners();
    await _preferences.setBool(_paddleOcrEnabledKey, value);
  }

  Future<void> setPaddleOcrApiKey(String value) async {
    _paddleOcrApiKeyValue = value.trim();
    notifyListeners();
    await _preferences.setString(_paddleOcrApiKey, _paddleOcrApiKeyValue);
  }
...
{content}
  ...
  SettingsProvider copyWith({
    ...
    copy._promptOptimizeModelProvider = _promptOptimizeModelProvider;
    copy._promptOptimizeModelId = _promptOptimizeModelId;
    copy._promptOptimizePrompt = _promptOptimizePrompt;
    ...
    copy._paddleOcrEnabled = _paddleOcrEnabled;
    copy._paddleOcrApiKeyValue = _paddleOcrApiKeyValue;
    copy._piAgentAutoUpdate = _piAgentAutoUpdate;
