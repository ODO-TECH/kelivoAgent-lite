...
class SkillsService extends ChangeNotifier {
  ...
  Future<void> _load() => _serializeMutation(() async {
    ...
    if (assets != null) {
      ...
      await _seedBundledSkill(await _ensureRoot(), assets, 'paddleocr');
    }
    await _rescan();
  });
  ...
  Future<void> _seedSkillCreator(Directory root, AssetBundle assets) async {
    ...
    await _seedBundledSkill(root, assets, id);
  }

  Future<void> _seedBundledSkill(
    Directory root,
    AssetBundle assets,
    String id,
  ) async {
    ...
    final receipt = File(p.join(root.path, '.bundled-$id'));
