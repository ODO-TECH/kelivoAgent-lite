...
class DesktopProcessRuntime extends WorkspaceRuntime {
  ...
  final Map<String, Future<_ShellSpec?>> _shells =
      <String, Future<_ShellSpec?>>{};
  ...
  Future<void> _execute(
    ...
    try {
      final spec = await _shell(request);
      if (spec == null) {
...
''';
  ...
  Future<_ShellSpec?> _shell([CommandRequest? request]) {
    final kind = request?.shellKind ?? 'automatic';
    final path = request?.shellPath.trim() ?? '';
    final key = '$kind\u0000$path';
    return _shells.putIfAbsent(key, () => _resolveShell(kind, path));
  }

  Future<_ShellSpec?> _resolveShell(String kind, String path) async {
    if (Platform.isWindows) {
      return _resolveWindowsShell(kind, path);
    }
    if (Platform.isMacOS || Platform.isLinux) {
      if (path.isNotEmpty) return _shellFromPath(path);
  ...
  Future<_ShellSpec?> _resolveWindowsShell(String kind, String path) async {
    if (kind == 'custom') return _shellFromPath(path);
    if (kind == 'gitBash') return _resolveGitBash(path);
    if (kind == 'powerShell') return _resolvePowerShell();
    if (kind == 'commandPrompt') {
      final cmd = await _which('cmd');
      return cmd == null ? null : _ShellSpec(cmd, _cmdArgs);
    }
    return _resolveAutomaticWindowsShell();
  }

  Future<_ShellSpec?> _resolveAutomaticWindowsShell() async {
    final powerShell = await _resolvePowerShell();
    if (powerShell != null) return powerShell;
    final cmd = await _which('cmd');
    return cmd == null ? null : _ShellSpec(cmd, _cmdArgs);
  }

  Future<_ShellSpec?> _resolvePowerShell() async {
  ...
  Future<_ShellSpec?> _resolveGitBash(String path) async {
    if (path.isNotEmpty) return _shellFromPath(path);
    final roots = <String>{
    ...
    for (final root in roots) {
      final candidates = <String>[
        ...
        p.join(root, 'Git', 'usr', 'bin', 'bash.exe'),
        p.join(root, 'Programs', 'Git', 'bin', 'bash.exe'),
      ];
      for (final candidate in candidates) {
        if (File(candidate).existsSync()) {
          return _ShellSpec(candidate, _bashArgs);
        }
      }
    }
    final bash = await _which('bash');
    return bash == null ? null : _ShellSpec(bash, _bashArgs);
  }

  _ShellSpec? _shellFromPath(String path) {
    if (path.isEmpty || !File(path).existsSync()) return null;
    final name = p.basename(path).toLowerCase();
    if (name == 'cmd' || name == 'cmd.exe') {
      return _ShellSpec(path, _cmdArgs);
    }
    if (name.contains('powershell') || name == 'pwsh' || name == 'pwsh.exe') {
      return _ShellSpec(path, _powerShellArgs);
    }
    if (name.contains('bash') || name == 'sh' || name.endsWith('sh.exe')) {
      return _ShellSpec(path, _bashArgs);
    }
    return _ShellSpec(path, _genericShellArgs);
  }
  ...
  List<String> _bashArgs(String command) => <String>[
    '--noprofile',
    '--norc',
    '-lc',
    command,
  ...
  List<String> _genericShellArgs(String command) => <String>['-c', command];
