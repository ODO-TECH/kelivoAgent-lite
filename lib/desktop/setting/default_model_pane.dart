...
import '../../core/services/agent/pi_agent_update_service.dart';
...
class DesktopDefaultModelPane extends StatelessWidget {
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  _ModelCard(
                    icon: lucide.Lucide.WandSparkles,
                    title: 'Prompt optimizer model',
                    subtitle:
                        'Rewrite a draft into a clear, executable prompt before sending.',
                    modelProvider: settings.promptOptimizeModelProvider,
                    modelId: settings.promptOptimizeModelId,
                    disabledWhenUnset: true,
                    showResetWhenUnset: true,
                    onReset: () async {
                      await context
                          .read<SettingsProvider>()
                          .resetPromptOptimizeModel();
                    },
                    onPick: () async {
                      final selected = await pickConfiguredModel(
                        settings.promptOptimizeModelProvider,
                        settings.promptOptimizeModelId,
                      );
                      if (selected != null) {
                        await context
                            .read<SettingsProvider>()
                    ...
                    configAction: () =>
                        _showPromptOptimizePromptDialog(context),
                  ),
                  ...
                  const SizedBox(height: 16),
                  const _PaddleOcrSettingsCard(),
                  const SizedBox(height: 16),
                  const _PiAgentSettingsCard(),
                ],
              ),
            ),
          ),
  ...
  Future<void> _showPromptOptimizePromptDialog(BuildContext context) async {
    final sp = context.read<SettingsProvider>();
    final ctrl = TextEditingController(text: sp.promptOptimizePrompt);
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Prompt optimizer instructions'),
        content: SizedBox(
          width: 620,
          height: 360,
          child: TextField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'Use {content} for the draft text.',
            ),
          ),
        ),
        actions: [
          TextButton(
            onPressed: () async {
              await sp.resetPromptOptimizePrompt();
          ...
          FilledButton(
            onPressed: () async {
              await sp.setPromptOptimizePrompt(ctrl.text);
              if (ctx.mounted) Navigator.of(ctx).pop();
            },
...
class _PaddleOcrSettingsCard extends StatefulWidget {
  const _PaddleOcrSettingsCard();

  @override
  State<_PaddleOcrSettingsCard> createState() => _PaddleOcrSettingsCardState();
}

class _PiAgentSettingsCard extends StatefulWidget {
  const _PiAgentSettingsCard();

  @override
  State<_PiAgentSettingsCard> createState() => _PiAgentSettingsCardState();
}

class _PiAgentSettingsCardState extends State<_PiAgentSettingsCard> {
  static const _service = PiAgentUpdateService();
  PiAgentUpdateInfo? _info;
  String? _status;
  bool _busy = false;
  ...
  Future<void> _check() async {
    ...
    try {
      ...
      setState(() {
        _info = info;
        _status = info.available
            ? 'Update available: ${info.latest}'
            : 'Already up to date';
      });
    } catch (error) {
      if (mounted) setState(() => _status = 'Check failed: $error');
    } finally {
  ...
  Future<void> _stage() async {
    ...
    setState(() => _busy = true);
    try {
      final file = await _service.stageLatest(info);
      if (mounted)
        setState(
          () => _status =
              'Staged ${file.path}. Restart/rebuild the sidecar to apply it.',
        );
    } catch (error) {
      if (mounted) setState(() => _status = 'Update failed: $error');
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(lucide.Lucide.RefreshCw, size: 18),
              ...
              IosSwitch(
                value: settings.piAgentAutoUpdate,
                semanticLabel: 'Automatically check for pi Agent updates',
                onChanged: settings.setPiAgentAutoUpdate,
              ),
            ],
          ),
          const SizedBox(height: 6),
          Text(
            'Current embedded version: ${_service.currentVersion}. Checking is opt-in; updates are staged in app data and never replace a running sidecar.',
            style: TextStyle(
              fontSize: 12,
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              FilledButton.icon(
                onPressed: _busy || _info?.available != true ? null : _stage,
                icon: const Icon(lucide.Lucide.Download, size: 16),
                label: const Text('Stage update'),
              ),
            ],
          ),
          if (_status != null)
...
class _PaddleOcrSettingsCardState extends State<_PaddleOcrSettingsCard> {
  ...
  void initState() {
    ...
    _keyController = TextEditingController(
      text: context.read<SettingsProvider>().paddleOcrApiKey,
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(lucide.Lucide.FileSearch, size: 18),
              const SizedBox(width: 8),
              const Expanded(
                child: Text(
                  'PaddleOCR skill',
              ...
              IosSwitch(
                value: settings.paddleOcrEnabled,
                semanticLabel: 'PaddleOCR skill',
                onChanged: settings.setPaddleOcrEnabled,
              ),
            ],
          ),
          const SizedBox(height: 6),
          Text(
            'Preinstalled layout-aware OCR skill. Enabled by default; add an AI Studio access token to use it.',
            style: TextStyle(
              fontSize: 12,
              color: cs.onSurface.withValues(alpha: 0.66),
            ),
          ...
          TextField(
            ...
            obscureText: true,
            onChanged: settings.setPaddleOcrApiKey,
            decoration: const InputDecoration(
              labelText: 'PaddleOCR API key',
              hintText: 'AI Studio access token',
              border: OutlineInputBorder(),
              isDense: true,
            ),
          ),
