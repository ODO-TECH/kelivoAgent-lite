...
import '../../../core/services/agent/agent_sidecar_service.dart';
...
class ChatActions {
  ...
  final AgentSidecarService _agentSidecar = AgentSidecarService();
  ...
  Future<void> _executeGeneration(stream_ctrl.GenerationContext ctx) async {
    ...
    try {
      ...
      if (ctx.agentEnabled) {
        final agentStream = _agentSidecar.run(
          requestId: conversationId,
          cwd: ctx.agentCwd ?? Directory.current.path,
          config: ctx.config,
          modelId: ctx.modelId,
          messages: ctx.apiMessages,
          prompt: _agentPrompt(ctx.apiMessages),
          tools: ctx.toolDefs,
          onToolCall: onToolCall,
          systemPrompt: ctx.agentSystemPrompt,
          paddleOcrApiKey: ctx.paddleOcrApiKey,
          useResponseApi: ctx.config.useResponseApi,
        );
  ...
  String _agentPrompt(List<Map<String, dynamic>> messages) {
