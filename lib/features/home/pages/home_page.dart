...
import '../../../core/services/chat/prompt_optimizer_service.dart';
...
import '../widgets/agent_syntax_highlighting_controller.dart';
...
class _HomePageState extends State<HomePage>
  ...
  final AgentSyntaxHighlightingController _inputController =
      AgentSyntaxHighlightingController();
  ...
  Widget _buildChatInputBar(BuildContext context, {required bool isTablet}) {
    ...
    return ChatInputSection(
      ...
      onOptimizePrompt: () => _optimizeDraftPrompt(context),
      onToggleOcr: () async {
        final sp = context.read<SettingsProvider>();
  ...
  Future<void> _optimizeDraftPrompt(BuildContext context) async {
    final draft = _inputController.text;
    if (draft.trim().isEmpty) return;
    final settings = context.read<SettingsProvider>();
    try {
      final optimized = await const PromptOptimizerService().optimize(
        settings: settings,
        draft: draft,
        conversationId: _controller.currentConversation?.id,
      );
      ...
      showAppSnackBar(
        context,
        message: 'Prompt optimization failed: $error',
        type: NotificationType.error,
      );
    }
  }
