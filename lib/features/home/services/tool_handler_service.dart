...
class ToolHandlerService {
  ...
  List<Map<String, dynamic>> buildToolDefinitions(
    ...
    // The PC Agent sidecar owns its own tool loop, so a workspace must expose
    // its tools even when Kelivo's model heuristics do not recognize the model
    // as tool-capable yet.
    if (workspaceContext != null && (supportsTools || Platform.isWindows)) {
