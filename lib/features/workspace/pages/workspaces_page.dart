...
class _WorkspaceSettingsFormState extends State<_WorkspaceSettingsForm> {
  ...
  TextEditingController? _shellPathController;
  bool? _shellNeedsApproval;
  WorkspaceShellKind? _shellKind;
  String? _defaultCwd;
  bool _cwdBusy = false;
  ...
  void didChangeDependencies() {
    ...
    _shellPathController ??= TextEditingController(text: workspace.shellPath);
    _shellNeedsApproval ??= workspace.shellNeedsApproval;
    _shellKind ??= workspace.shellKind;
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty) return;
    final shellKind = _shellKind ?? workspace.shellKind;
    final shellPath = _shellPathController?.text.trim() ?? workspace.shellPath;
    if (shellKind == WorkspaceShellKind.custom && shellPath.isEmpty) return;
    await context.read<WorkspaceProvider>().update(
      workspace.copyWith(
        name: name,
  ...
  Widget build(BuildContext context) {
    ...
    final canSave =
        ...
        (shellKind != WorkspaceShellKind.custom ||
            shellPathController.text.trim().isNotEmpty);
    final children = <Widget>[
      SectionCard(
        children: [
          IosFormTextField(
      ...
      SectionCard(
        children: [
          ...
          if (Platform.isWindows || Platform.isMacOS || Platform.isLinux) ...[
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Text(
                      l10n.workspacesShell,
                      style: const TextStyle(fontSize: 15),
                    ),
                  ),
                  const SizedBox(width: 12),
                  DropdownButtonHideUnderline(
                    child: DropdownButton<WorkspaceShellKind>(
                      value: shellKind,
                      onChanged: (value) {
                        if (value != null) setState(() => _shellKind = value);
                      },
                      items: WorkspaceShellKind.values
                          .map(
                            (kind) => DropdownMenuItem<WorkspaceShellKind>(
                              value: kind,
                              child: Text(_shellLabel(l10n, kind)),
                            ),
                          )
                          .toList(),
                    ),
            ...
            if (shellKind == WorkspaceShellKind.gitBash ||
                shellKind == WorkspaceShellKind.custom) ...[
              const IosRowDivider(),
              IosFormTextField(
                label: l10n.workspaceEnvShellPath,
                controller: shellPathController,
                hintText: l10n.workspacesShellPathHint,
                inlineLabel: false,
                autocorrect: false,
...
String _shellLabel(AppLocalizations l10n, WorkspaceShellKind kind) {
  return switch (kind) {
    WorkspaceShellKind.automatic => l10n.workspaceEnvShellAutomatic,
    WorkspaceShellKind.powerShell => l10n.workspacesShellPowerShell,
    WorkspaceShellKind.commandPrompt => l10n.workspacesShellCommandPrompt,
    WorkspaceShellKind.gitBash => l10n.workspacesShellGitBash,
    WorkspaceShellKind.custom => l10n.workspacesShellCustom,
  };
}
