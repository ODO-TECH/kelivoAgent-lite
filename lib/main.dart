      ...
      providers: [
        ...
        ChangeNotifierProvider(
          create: (_) {
            ...
            unawaited(
              settings.loaded.then((_) async {
                await settings.incrementAppLaunchCount();
                if (!Platform.isWindows || !settings.piAgentAutoUpdate) {
                  return;
                }
                try {
                  const updater = PiAgentUpdateService();
                  final info = await updater.checkLatest();
                  if (info.available) await updater.stageLatest(info);
                } catch (error) {
                  debugPrint('pi Agent update check failed: $error');
                }
              }),
            );
            return settings;
