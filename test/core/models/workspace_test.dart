...
void main() {
  group('Workspace', () {
    ...
    test('defaults shellNeedsApproval and defaultCwd', () {
      ...
      expect(workspace.shellKind, WorkspaceShellKind.automatic);
      expect(workspace.shellPath, isEmpty);
    });

    test('JSON round trip', () {
      final workspace = sample().copyWith(
        shellKind: WorkspaceShellKind.gitBash,
        shellPath: r'C:\Program Files\Git\bin\bash.exe',
      );
      final decoded = Workspace.fromJson(workspace.toJson());
      expect(decoded, workspace);
    });

    test('reads legacy JSON without shell fields', () {
      final decoded = Workspace.fromJson(
        sample().toJson()
          ..remove('shellKind')
          ..remove('shellPath'),
      );
      expect(decoded.shellKind, WorkspaceShellKind.automatic);
