...
void main() {
  ...
  testWidgets('workspace tools appear only when a context is passed', (
    ...
    if (Platform.isWindows) {
      // Windows workspaces use the pi sidecar Agent loop, so the workspace
      // tools remain available even when the provider model heuristic is false.
      expect(noTools.map(nameOf), containsAll(WorkspaceToolsService.toolNames));
    } else {
      for (final name in WorkspaceToolsService.toolNames) {
